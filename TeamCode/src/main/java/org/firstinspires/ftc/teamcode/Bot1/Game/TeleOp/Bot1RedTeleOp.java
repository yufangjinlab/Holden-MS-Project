...
@TeleOp(name = "Bot 1 TeleOp: Red", group = "TeleOp")
public class Bot1RedTeleOp extends LinearOpMode {
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            if (result.isValid()){
                if (result.getFiducialResults().get(0).getFiducialId() == 24){
                    target = result.getTx();
                    if(target <= turret.RED_MAX_RIGHT - 8){
                        turret.turret.setPower(-0.11);
                    } else if (target >= turret.RED_MAX_LEFT + 8){
                        turret.turret.setPower(0.13);
                    }

                    if(target <= turret.RED_MAX_RIGHT && target > turret.RED_MAX_RIGHT - 8){
                        turret.turret.setPower(-0.05);
                    } else if (target >= turret.RED_MAX_LEFT && target < turret.RED_MAX_LEFT + 8){
                        turret.turret.setPower(0.07);
                    } else if (target < turret.RED_MAX_LEFT && target > turret.RED_MAX_RIGHT){
...
//
//            telemetry.addData("Max Right: ", turret.RED_MAX_RIGHT);
//            telemetry.addData("Max Left: ", turret.RED_MAX_LEFT);
//
